...
public class CheckPassState extends AbstractState {
    ...
    public Result arraignment(Long activityId, Enum<ActivityConstants.ActivityState> currentStatus) {
        return Result.buildResult(Constants.ResponseCode.UNKNOWN_ERROR,"已审核状态不能重复提交");
    }
    ...
    public Result checkRefuse(Long activityId, Enum<ActivityConstants.ActivityState> currentStatus) {
        boolean isSuccess = activityRepository.alterStatus(activityId, currentStatus, ActivityConstants.ActivityState.REFUSE);
    ...
    public Result doing(Long activityId, Enum<ActivityConstants.ActivityState> currentState) {
        boolean isSuccess = activityRepository.alterStatus(activityId, currentState, ActivityConstants.ActivityState.DOING);
